...
@pytest.mark.skipif(os.getenv("CI") == "true", reason="Live server not running in CI")
@pytest.mark.asyncio
async def test_real_baseline_e2e_flow() -> None:
    ...
    async with httpx.AsyncClient(base_url=API_URL, timeout=30.0) as client:
        ...
        assert res_ready.status_code == 200, f"Readiness probe failed: HTTP {res_ready.status_code} - {res_ready.text}"  # noqa: E501
        print("[PASS] Step 2: Readiness /health/ready = 200 OK")

        # Step 3: System Status
        res_sys = await client.get("/api/v1/system/status")
        assert res_sys.status_code == 200, f"System status probe failed: HTTP {res_sys.status_code}"  # noqa: E501
        ...
        assert res_me.status_code == 200, f"User identity fetch failed: HTTP {res_me.status_code} - {res_me.text}"  # noqa: E501
        ...
        assert res_upload.status_code in [200, 201], f"Dataset upload failed: HTTP {res_upload.status_code} - {res_upload.text}"  # noqa: E501
        upload_json = res_upload.json()
        dataset_data = upload_json["dataset"]
        dataset_id = dataset_data["id"]
        prof_job_id = upload_json.get("job_id")
        print(f"[PASS] Step 5: Dataset customer_churn.csv uploaded (ID: {dataset_id}, Profiling Job ID: {prof_job_id})")  # noqa: E501

        # Poll real background worker for profiling job completion
        prof_completed = False
        for _ in range(20):
            res_j = await client.get(f"/api/v1/jobs/{prof_job_id}?workspace_id={workspace_id}", headers=headers)  # noqa: E501
            if res_j.status_code == 200:
                ...
                if j_status == "completed":
                    ...
                    raise RuntimeError(f"Profiling worker job failed: {res_j.json().get('error_message')}")  # noqa: E501
        ...
        assert res_train.status_code in [200, 201, 202], f"Training submit failed: HTTP {res_train.status_code} - {res_train.text}"  # noqa: E501
        ...
        for _ in range(30):
            res_j = await client.get(f"/api/v1/jobs/{train_job_id}?workspace_id={workspace_id}", headers=headers)  # noqa: E501
            if res_j.status_code == 200:
                j_status = res_j.json()["status"]
                if j_status == "completed":
                    ...
                    raise RuntimeError(f"Training worker job failed: {res_j.json().get('error_message')}")  # noqa: E501
            await asyncio.sleep(1.0)

        assert train_completed, f"Training job {train_job_id} did not complete within timeout"
        print(f"[PASS] Step 7b: Real background worker processed and completed training job {train_job_id}")  # noqa: E501

        # Step 8: Verify Model Appears in Registry
        res_models = await client.get(f"/api/v1/models?workspace_id={workspace_id}", headers=headers)  # noqa: E501
        ...
        print(f"[PASS] Step 8: Model registered in Model Registry (ID: {model_id}, Metrics: {target_model['metrics_json']})")  # noqa: E501
        ...
        assert res_eval.status_code == 200, f"Model evaluation failed: HTTP {res_eval.status_code} - {res_eval.text}"  # noqa: E501
        print(f"[PASS] Step 8b: Model evaluation passed quality gate (Status: {res_eval.json()['status']})")  # noqa: E501
        ...
        assert res_promote.status_code == 200, f"Model promotion API failed: HTTP {res_promote.status_code} - {res_promote.text}"  # noqa: E501
        promoted_model = res_promote.json()
        assert promoted_model["status"] == "production", f"Model status is {promoted_model['status']}, expected 'production'"  # noqa: E501
        print("[PASS] Step 9: Model promoted to production via API promotion workflow")
        ...
        assert res_pred.status_code == 200, f"Inference API failed: HTTP {res_pred.status_code} - {res_pred.text}"  # noqa: E501
        ...
        print(f"[PASS] Step 10: Inference succeeded: prediction={pred_out['prediction']}, confidence={pred_out['confidence']}, latency={pred_out['latency_ms']}ms")  # noqa: E501
        ...
        assert res_telemetry.status_code == 200, f"Inference logs failed: HTTP {res_telemetry.status_code}"  # noqa: E501
