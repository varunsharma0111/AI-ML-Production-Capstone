...
class MLService:
    ...
    async def register_and_train_model(self, session: AsyncSession, principal: Principal, payload: ModelCreate) -> ModelVersion:
        if await _is_in_transaction(session):
            return await self._register_and_train_model_impl(session, principal, payload)
        async with session.begin():
            return await self._register_and_train_model_impl(session, principal, payload)

    async def _register_and_train_model_impl(self, session: AsyncSession, principal: Principal, payload: ModelCreate) -> ModelVersion:
    ...
    async def list_models(self, session: AsyncSession, principal: Principal, workspace_id: UUID | None = None) -> list[ModelVersion]:
        if await _is_in_transaction(session):
            return await self._list_models_impl(session, principal, workspace_id)
        async with session.begin():
            return await self._list_models_impl(session, principal, workspace_id)

    async def _list_models_impl(self, session: AsyncSession, principal: Principal, workspace_id: UUID | None = None) -> list[ModelVersion]:
        if workspace_id:
    ...
    async def get_model(
        ...
        if await _is_in_transaction(session):
            return await self._get_model_impl(session, principal, model_id, workspace_id)
        async with session.begin():
            return await self._get_model_impl(session, principal, model_id, workspace_id)

    async def _get_model_impl(
        self,
        session: AsyncSession,
        principal: Principal,
        model_id: UUID,
    ...
    async def evaluate_model(
        ...
        if await _is_in_transaction(session):
            return await self._evaluate_model_impl(session, principal, model_id, payload, request_id)
        async with session.begin():
            return await self._evaluate_model_impl(session, principal, model_id, payload, request_id)
    ...
    async def promote_model(
        ...
        if await _is_in_transaction(session):
            return await self._promote_model_impl(session, principal, model_id, payload, request_id)
        async with session.begin():
            return await self._promote_model_impl(session, principal, model_id, payload, request_id)

    async def _promote_model_impl(
        self,
        session: AsyncSession,
        principal: Principal,
        model_id: UUID,
    ...
    async def get_quality_gate(self, session: AsyncSession, principal: Principal, model_id: UUID, workspace_id: UUID) -> QualityGateResponse:
        if await _is_in_transaction(session):
            return await self._get_quality_gate_impl(session, principal, model_id, workspace_id)
        async with session.begin():
            return await self._get_quality_gate_impl(session, principal, model_id, workspace_id)

    async def _get_quality_gate_impl(self, session: AsyncSession, principal: Principal, model_id: UUID, workspace_id: UUID) -> QualityGateResponse:
        await self._authorized_user(session, principal, workspace_id, Permission.MODEL_READ)
        model = await self._model_repository.get_model_version(session, model_id)
        if model is None:
            raise ResourceNotFoundError("Model version not found.")
    ...
    async def predict(
        ...
        if await _is_in_transaction(session):
            return await self._predict_impl(session, principal, model_id, payload, request_id)
        async with session.begin():
            return await self._predict_impl(session, principal, model_id, payload, request_id)
    ...
    async def list_inference_logs(
        ...
        if await _is_in_transaction(session):
            return await self._list_inference_logs_impl(session, principal, workspace_id, limit)
        async with session.begin():
            return await self._list_inference_logs_impl(session, principal, workspace_id, limit)

    async def _list_inference_logs_impl(
        self,
        session: AsyncSession,
        principal: Principal,
        workspace_id: UUID,
